(ns clj-camel.util
  ...
  (:import (java.util.concurrent TimeUnit)
    ...
    #_(utils MDCFromHeadersUnitOfWorkFactory)))
...
(defn debug-route [{:keys [ctx headers body]} route]
  (let [res                              (atom nil)
        ...
        ^ManagedCamelContext managed-ctx (-> (.getCamelContextExtension ctx)
                                             (.getContextPlugin ManagedCamelContext))
...
#_(defn set-customer-uow-with-mdc-from-headers [^DefaultCamelContext context mapping]
  {:pre [(map? mapping)]}
  (-> (.getCamelContextExtension context)
      (.addContextPlugin UnitOfWorkFactory (MDCFromHeadersUnitOfWorkFactory. mapping))))